...
local function build(dir, makeargs, defines, faces, secondary_face_index)
  ...
  local ok, stdout, stderr, reason, status = shell.run([[PATH=/bin:/usr/bin exec /code/build.sh "]] .. dir .. [[" ]] .. makeargs['BOARD'] .. ' '  .. makeargs['DISPLAY'] .. ' ' .. table.concat(argslist, ' '), nil, 20000)
  if ok == nil then
